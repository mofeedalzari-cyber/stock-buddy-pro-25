// ============================================================================
// ملف: src/pages/movements/ReportsPage.tsx (نسخة مصححة بالكامل - إصلاح ترويسة الجداول)
// ============================================================================
...
const ReportsPage = () => {
    ...
    return targetClients.flatMap(client => {
      ...
      return clientEntitlements.map(ent => {
        ...
        monthMovements.forEach(m => {
          ...
          if (m.items) m.items.forEach(item => { if (item.product_id === ent.product_id) actualQty += (item.quantity ?? 0); });
        });
        const baseEntitlement = ent.monthly_quantity;
        const exceeded = actualQty > baseEntitlement;
        return {
          clientId: client.id,
          ...
          actual: formatQuantityWithUnit(actualQty, product),
          remaining: formatQuantityWithUnit(Math.max(0, baseEntitlement - actualQty), product),
          exceeded,
          overAmount: exceeded ? actualQty - baseEntitlement : 0,
          unit: product.display_unit_id ? getUnitName(product.display_unit_id) : (product.unit || 'قطعة'),
        };
      }).filter(Boolean);
    });
  ...
  const printSelectedClientEntitlements = () => {
    if (!selectedClient || !checkWarehouseSelected()) return;
    const clientName = clients.find(c => c.id === selectedClient)?.name || 'غير محدد';
    const rows = entitlementReport.filter(r => r.clientId === selectedClient).map((r: any, i: number) => [
      String(i + 1), r.productName, r.entitlement, r.actual, r.remaining, r.exceeded ? String(r.overAmount) : '-', r.exceeded ? 'خارج الاستحقاق' : 'ضمن الاستحقاق', r.unit,
    ]);
    const html = buildSimplePdfHtml(`تقرير استحقاقات ${clientName}`, ['م', 'المنتج', 'الاستحقاق', 'المصروف', 'المتبقي', 'الزيادة', 'الحالة', 'الوحدة'], rows, selectedWarehouse, getWarehouseName, warehouseManager, displayName || '__________');
  ...
  const exportEntitlementsPdf = () => {
    ...
    const rows = entitlementReport.map((r: any, i: number) => [
      String(i + 1), r.clientName, r.productName, r.entitlement, r.actual, r.remaining, r.exceeded ? String(r.overAmount) : '-', r.exceeded ? 'خارج الاستحقاق' : 'ضمن الاستحقاق', r.unit,
    ]);
    const html = buildSimplePdfHtml('تقرير الاستحقاقات', ['م', 'جهة الصرف', 'المنتج', 'الاستحقاق', 'المصروف', 'المتبقي', 'الزيادة', 'الحالة', 'الوحدة'], rows, selectedWarehouse, getWarehouseName, warehouseManager, displayName || '__________');
    printPdfFromHtml(html, 'تقرير_الاستحقاقات', toast);
  };

  const exportEntitlementsExcel = () => {
    if (!checkWarehouseSelected()) return;
    exportExcel(entitlementReport.map((r: any) => ({ 'جهة الصرف': r.clientName, 'المنتج': r.productName, 'الاستحقاق': r.entitlement, 'المصروف': r.actual, 'المتبقي': r.remaining, 'الحالة': r.exceeded ? 'خارج الاستحقاق' : 'ضمن الاستحقاق' })), 'الاستحقاقات', 'تقرير_الاستحقاقات');
  };
    ...
    <div className="space-y-4 sm:space-y-5" dir="rtl">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <select value={selectedWarehouse} onChange={e => setSelectedWarehouse(e.target.value)} className="flex-1 h-10 rounded-md border border-input bg-background px-3 text-sm focus:ring-2 focus:ring-ring">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all shrink-0 ${tab === t.id ? 'gradient-primary text-primary-foreground shadow-md' : 'bg-card text-muted-foreground border border-border hover:bg-secondary'}`}>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-card rounded-xl p-4 border border-border text-center">
              <div className="text-xl font-bold text-foreground">{filteredProducts.length}</div>
              <div className="text-xs text-muted-foreground mt-1">إجمالي المنتجات</div>
            </div>
            <div className="bg-card rounded-xl p-4 border border-border text-center">
              <div className="text-xl font-bold text-foreground">{filteredProducts.reduce((s, p) => s + getDisplayQty(p), 0)}</div>
              <div className="text-xs text-muted-foreground mt-1">إجمالي المخزون</div>
            </div>
            <div className="bg-card rounded-xl p-4 border border-border text-center">
              <div className="text-xl font-bold text-foreground">{categories.length}</div>
              <div className="text-xs text-muted-foreground mt-1">الأصناف</div>
            </div>
          </div>
          <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-border bg-secondary/20">
              <h3 className="font-semibold text-sm">جدول المنتجات</h3>
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={exportProductsExcel} className="h-8 text-xs"><FileSpreadsheet className="w-3.5 h-3.5 ml-1" /> Excel</Button>
                <Button size="sm" variant="outline" onClick={exportProductsPdf} className="h-8 text-xs"><FileText className="w-3.5 h-3.5 ml-1" /> PDF</Button>
              </div>
            </div>
                ...
                <tbody>
                  {filteredProducts.map((p, i) => (
                    <tr key={p.id} className="border-b border-border hover:bg-secondary/20">
                      ...
                      <td className="p-3">{getCategoryName(p.category_id || '')}</td>
                      <td className="p-3 font-bold">{getFormattedQuantityForProduct(p)}</td>
                      <td className="p-3 text-muted-foreground">{p.display_unit_id ? getUnitName(p.display_unit_id) : (p.unit || 'قطعة')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-4">
          <div className="bg-card rounded-xl p-4 border border-border flex flex-wrap gap-4 items-end">
            ...
            <div className="flex rounded-lg border border-border overflow-hidden h-9">
              {['all', 'in', 'out'].map(f => (
                <button key={f} onClick={() => setMovementFilter(f as any)} className={`px-4 text-xs font-medium ${movementFilter === f ? 'bg-primary text-primary-foreground' : 'bg-card text-muted-foreground hover:bg-secondary'}`}>
          ...
          <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 className="font-semibold text-sm">سجل الحركات</h3>
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={exportMovementsExcel} className="h-8 text-xs"><FileSpreadsheet className="w-3.5 h-3.5 ml-1" /> Excel</Button>
                <Button size="sm" variant="outline" onClick={exportMovementsPdf} className="h-8 text-xs"><FileText className="w-3.5 h-3.5 ml-1" /> PDF</Button>
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
        ...
        <div className="space-y-4">
          <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
            <div className="p-4 border-b border-border bg-secondary/20 flex justify-between items-center">
              <h3 className="font-semibold text-sm">ملخص المخازن</h3>
              <Button size="sm" variant="outline" onClick={exportWarehousesPdf} className="h-8 text-xs"><Printer className="w-3.5 h-3.5 ml-1" /> طباعة</Button>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-secondary/50">
      ...
      {tab === 'low-stock' && (
        <div className="space-y-4">
          <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden border-red-200">
            <div className="p-4 border-b border-red-100 bg-red-50 flex items-center justify-between">
              <h3 className="font-semibold text-sm text-red-700 flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> تنبيهات المخزون المنخفض</h3>
              <Button size="sm" variant="outline" onClick={exportLowStockPdf} className="h-8 text-xs border-red-200 text-red-700 hover:bg-red-100"><FileText className="w-3.5 h-3.5 ml-1" /> تصدير التقرير</Button>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {lowStock.map((p, i) => (
                    <tr key={i} className="border-b border-border hover:bg-red-50/30">
                      <td className="p-3 font-medium">{p.name}</td>
                      <td className="p-3 font-bold text-red-600">{getFormattedQuantityForProduct(p)}</td>
                      <td className="p-3"><span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-red-100 text-red-700">منخفض جداً</span></td>
                    </tr>
                  ))}
                  {lowStock.length === 0 && (
                    <tr><td colSpan={3} className="p-8 text-center text-muted-foreground">لا توجد منتجات منخفضة حالياً.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      {tab === 'entities' && (
        <div className="space-y-4">
          <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
            <div className="p-4 border-b border-border bg-secondary/20 flex justify-between items-center">
              <h3 className="font-semibold text-sm flex items-center gap-2"><Users className="w-4 h-4" /> سجل حركات الموردين وجهات الصرف</h3>
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={printSuppliersDetails} className="h-8 text-xs">حركات الموردين</Button>
                <Button size="sm" variant="outline" onClick={printClientsDetails} className="h-8 text-xs">حركات جهات الصرف</Button>
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {expanded.filter(m => m.entity_type).map((m, i) => (
                    <tr key={i} className="border-b border-border hover:bg-secondary/20">
                      ...
                      <td className="p-3 font-bold">{getFormattedMovementQty(m)}</td>
                      <td className="p-3 text-muted-foreground text-xs">{getWarehouseName(m.warehouse_id)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-4">
          <div className="bg-card rounded-xl p-4 border border-border flex flex-wrap gap-4 items-end">
            ...
            <div className="space-y-1">
              ...
              <select value={selectedClient} onChange={e => setSelectedClient(e.target.value)} className="h-9 rounded-md border border-input bg-background px-3 text-sm focus:ring-2 focus:ring-ring w-48">
                <option value="">-- الكل --</option>
                {clientsWithEntitlements.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
            <div className="flex gap-2 mr-auto">
              <Button size="sm" variant="outline" onClick={exportEntitlementsExcel} className="h-8 text-xs">Excel</Button>
              <Button size="sm" variant="outline" onClick={exportEntitlementsPdf} className="h-8 text-xs">PDF</Button>
            </div>
          </div>
          <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-secondary/50">
                  <tr className="border-b border-border">
                    <th className="text-right p-3">جهة الصرف</th>
                    <th className="text-right p-3">المنتج</th>
                ...
                <tbody>
                  {entitlementReport.map((r: any, i: number) => (
                    <tr key={i} className={`border-b border-border hover:bg-secondary/20 ${r.exceeded ? 'bg-red-50/50' : ''}`}>
